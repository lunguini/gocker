...
- [ ] `/containers/{id}/attach` — hijacked-TCP bidirectional stdio multiplexer required for `docker run -it` / `docker attach`. Large: HTTP upgrade, 8-byte stream headers for stdout/stderr framing, TTY raw-mode pty wiring, signal handling. `docker run -d` works without it. Deserves its own plan. Surfaced by real docker CLI: `docker run alpine:3 echo hi` returns `unable to upgrade to tcp, received 404`.
- [ ] `/containers/{id}/wait` — endpoint that blocks until the container exits and returns the exit code. Used by `docker run -d` (even detached), `docker wait`, and most orchestration tooling. Small compared to `/attach` — it's a single long-poll rather than a stream. Should come alongside attach.
