...
**Planned work** is tracked as GitHub issues with the `roadmap` label:
[github.com/lunguini/gocker/issues?q=label%3Aroadmap](https://github.com/lunguini/gocker/issues?q=label%3Aroadmap).

Current top items: Docker API `/containers/{id}/attach` for `docker run -it`, `/containers/{id}/wait`
for `docker run -d` exit codes, image-pull performance tuning, sandbox network-policy enforcement,
and Codex/Gemini agent templates.
