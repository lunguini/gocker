...
- **`--isolation` flag is compose-only.** Runtimes are constructed from config before flag parsing, so the global `--isolation` flag cannot re-route non-compose commands; those now error with a pointer to `~/.gocker/config.yaml`. Full flag-driven re-resolution needs a swappable-Runtime indirection (tracked in `.docs/plan-1.md` wave 2).
