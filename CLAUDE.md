...
- **Shared/compose VMs need `--cap-add ALL`.** Apple CLI 1.1.0+ confines container processes to Docker's default capability set (no CAP_SYS_ADMIN), which breaks nested containerd (overlayfs/bind mounts fail "operation not permitted"). `sharedvm.Manager.capAddArgs` probes `run --help` for `--cap-add` support and adds `--cap-add ALL` to VM create args; pre-1.1.0 CLIs reject the flag but granted full caps implicitly. Any new code that creates a VM intended to run nested containers must include these args.
